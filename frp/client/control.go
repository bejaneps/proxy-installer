...
import (
	...
	"github.com/bejaneps/frp/client/proxy"
...
func (ctl *Control) Close() error {
	...
	values := map[string]string{"username": ctl.clientCfg.Username, "password": ctl.clientCfg.Password}
	jsonValues, err := json.Marshal(values)
	if err != nil {
		panic("control.go: couldn't unmarshal json " + err.Error())
	}

	_, err = http.Post(ctl.clientCfg.DisconnectURL, "application/json", bytes.NewBuffer(jsonValues))
	if err != nil {
		panic("control.go: couldn't send POST request " + err.Error())
	}
