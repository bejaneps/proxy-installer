...
func (pxy *BaseProxy) SetLoginData(msg msg.Login) {
	pxy.data = msg
}
...
func (pxy *BaseProxy) Close() {
	...
	if pxy.data.Username != "" && pxy.data.Password != "" && pxy.data.DisconnectURL != "" {
		/* sending u/p to disconnect server, so it frees u/p and ports in use */
		values := map[string]string{"username": pxy.data.Username, "password": pxy.data.Password}
		jsonValues, err := json.Marshal(values)
		if err != nil {
			panic("proxy close: couldn't unmarshal json " + err.Error())
		}

		_, err = http.Post(pxy.data.DisconnectURL, "application/json", bytes.NewBuffer(jsonValues))
		if err != nil {
			panic("proxy close: couldn't send POST request " + err.Error())
		}
	}
