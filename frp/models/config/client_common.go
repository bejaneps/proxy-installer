...
// BasicUserInfo represents info from .ini file
...
// GetDataFromIni returns basic user info, like u/p and urls
func GetDataFromIni(path string) (b BasicUserInfo, err error) {
	cfg, err := inigo.Load(path)
	if err != nil {
		err = fmt.Errorf("Parse ini conf file error: %v", err)
		return b, err
	}

	b.Username = cfg.Section("common").Key("username").String()
	if b.Username == "" {
		err = fmt.Errorf("Parse conf error: username is empty")
		return b, err
	}

	b.Password = cfg.Section("common").Key("password").String()
	if b.Password == "" {
		err = fmt.Errorf("Parse conf error: password is empty")
		return b, err
	}

	b.AuthURL = cfg.Section("common").Key("auth_url").String()
	if b.AuthURL == "" {
		err = fmt.Errorf("Parse conf error: auth_url is empty")
		return b, err
	}
	...
	if b.DisconnectURL == "" {
		err = fmt.Errorf("Parse conf error: disconnect_url is empty")
		return b, err
	}
...
// DelDataFromIni deletes sections from .ini file that were added after authentication process
func DelDataFromIni(path, username string, data []msg.Datum) error {
	cfg, err := inigo.Load(path)
	if err != nil {
		err = fmt.Errorf("Parse ini conf file error: %v", err)
		return err
	...
	for i := range data {
		cfg.DeleteSection(fmt.Sprintf("%s_tcp_%d", username, data[i].ClientPort))
		cfg.SaveTo(path)
	}
...
// AddDataToIni adds to frpc .ini file local_port and remote_port of client + encryption
func AddDataToIni(path string, data []msg.Datum, username string) error {
	cfg, err := inigo.Load(path)
	if err != nil {
		err = fmt.Errorf("Parse ini conf file error: %v", err)
		return err
	...
	for i := range data {
		sec, err := cfg.NewSection(fmt.Sprintf("%s_tcp_%d", username, data[i].ClientPort))
		if err != nil {
			err = fmt.Errorf("Editing ini conf file error: %v", err)
			return err
		}

		var encryption string
		if data[i].EnableEncryption {
			encryption = "true"
		} else {
			encryption = "false"
		}
		localPort := strconv.Itoa(data[i].ClientPort)
		remotePort := strconv.Itoa(data[i].ServerPort)

		cfg.Section(sec.Name()).Key("type").SetValue("tcp")
		cfg.Section(sec.Name()).Key("local_ip").SetValue("127.0.0.1")
		cfg.Section(sec.Name()).Key("local_port").SetValue(localPort)
		cfg.Section(sec.Name()).Key("remote_port").SetValue(remotePort)
		cfg.Section(sec.Name()).Key("use_encryption").SetValue(encryption)
		cfg.SaveTo(path)
	}
